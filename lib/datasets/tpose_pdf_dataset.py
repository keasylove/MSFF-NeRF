...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        joints = np.load(os.path.join(self.lbs_root, 'joints.npy')) #(24, 3)关节点位置
        self.joints = joints.astype(np.float32)
        self.parents = np.load(os.path.join(self.lbs_root, 'parents.npy')) #(24)
        weights = np.load(os.path.join(self.lbs_root, 'weights.npy')) #(6890,24)关节点对顶点影响的权重
    ...
    def prepare_input(self, i):
        ...
        if not os.path.exists(vertices_path):
            vertices_path = os.path.join(self.data_root, cfg.vertices,
                    '{:06d}.npy'.format(i))
        wxyz = np.load(vertices_path).astype(np.float32) #世界坐标系i帧顶点坐标
        ...
        pxyz = np.dot(wxyz - Th, R).astype(np.float32) #smpl坐标系i帧顶点坐标

        # calculate the skeleton transformation
        poses = params['poses'].reshape(-1, 3) #i帧各子节点相对父节点的旋转角
        joints = self.joints
        parents = self.parents
        A, canonical_joints = if_nerf_dutils.get_rigid_transformation(
            poses, joints, parents, return_joints=True) #i帧姿态下关节点的旋转矩阵以及变换后的关节位置(世界坐标系下)

        posed_joints = np.dot(canonical_joints, R.T) + Th #i帧变换后的关节位置(smpl坐标系下)

        # find the nearest training frame
        if cfg.test_novel_pose and hasattr(self, "training_joints"):
    ...
    def __getitem__(self, index):
        ...
        tvertices = np.load(vertices_path).astype(np.float32) #(6890, 3) bigpose的顶点
        tbounds = if_nerf_dutils.get_bounds(tvertices)

        wpts, pvertices, A, Rh, Th, poses, nearest_frame_index = self.prepare_input(i)

        # add
        # construct the tpose coordinate
        # # R_smpl = cv2.Rodrigues(Rh)[0].astype(np.float32)
        # # ptvertices = np.dot(tvertices - Th, R_smpl).astype(np.float32) #smpl下tvertices
        # ptvertices = tvertices
        # ptbounds = if_nerf_dutils.get_bounds(ptvertices)
        #
        # dhw = ptvertices[:, [2, 1, 0]]
        # min_dhw = ptbounds[0][[2, 1, 0]]
        # max_dhw = ptbounds[1][[2, 1, 0]]
        # voxel_size = np.array(cfg.voxel_size)
        # coord_vertices = np.round((dhw - min_dhw) / voxel_size).astype(np.int32)
        ...
        # add
        # construct the ppose coordinate
        dhw = pvertices[:, [2, 1, 0]]
        min_dhw = pbounds[0][[2, 1, 0]]
        max_dhw = pbounds[1][[2, 1, 0]]
        ptbounds = pbounds
        voxel_size = np.array(cfg.voxel_size)
        coord_vertices = np.round((dhw - min_dhw) / voxel_size).astype(np.int32)
        # construct the output shape
        ...
        rgb, ray_o, ray_d, near, far, coord, mask_at_box, radii = if_nerf_dutils.sample_ray_h36m(
        ...
        # nerf
        ret = {
            'coord_vertices': coord_vertices, #add
            'out_sh': out_sh, #add
            'rgb': rgb,
            'occupancy': occupancy,
            'ray_o': ray_o,
        ...
        meta = {
            ...
            'tvertices': tvertices,
            'pvertices': pvertices, #smpl坐标系下i帧姿态的顶点坐标
            'pbounds': pbounds,
            'wbounds': wbounds,
            'tbounds': tbounds,
            'ptbounds': ptbounds

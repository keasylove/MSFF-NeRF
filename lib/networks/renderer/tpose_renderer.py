...
class Renderer:
    ...
    def get_wsampling_points(self, ray_o, ray_d, radii, near, far):
        # calculate the steps for each ray
        t_vals = torch.linspace(0., 1., steps=cfg.N_samples+1).to(near)
        ...
        means, covs = cast_rays(z_vals, ray_o, ray_d, radii, 'cone')

        return means, z_vals[..., :cfg.N_samples], covs, (means.squeeze(0), covs.squeeze(0))
        # return means, 0.5*(z_vals[..., 1:]-z_vals[..., :-1]), covs, (means.squeeze(0), covs.squeeze(0))

    def get_density_color(self, wpts, covs, viewdir, z_vals, raw_decoder):
        """
        ...
        covs = covs.view(n_batch * n_pixel * n_sample, -1)
        ...
        ret = raw_decoder(wpts, covs, viewdir, dists)

        return ret

    def get_pixel_value(self, ray_o, ray_d, radii, near, far, occ, batch):
        n_batch = ray_o.shape[0]

        # sampling points for nerf training
        wpts, z_vals, covs, means_covs = self.get_wsampling_points(ray_o, ray_d, radii, near, far)
        n_batch, n_pixel, n_sample = wpts.shape[:3]

        # viewing direction, ray_d has been normalized in the dataset
        viewdir = ray_d

        raw_decoder = lambda wpts_val, covs, viewdir_val, dists_val: self.net(
            wpts_val, covs, viewdir_val, dists_val, batch, means_covs)

        # compute the color and density
        ret = self.get_density_color(wpts, covs, viewdir, z_vals, raw_decoder)

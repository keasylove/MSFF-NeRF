...
def sample_ray_h36m(img, msk, K, R, T, bounds, nrays, split):
    ...
    #add
    dx = np.sqrt(np.sum((ray_d[:-1, :, :] - ray_d[1:, :, :]) ** 2, -1))  # (511,512)
    dx = np.concatenate([dx, dx[-2:-1, :]], 0)  # (512,512)

    radii = dx[..., None] * 2 / np.sqrt(12)  # (512,512,1)
    ...
    if split == 'train':
        ...
        while nsampled_rays < nrays:
            ...
            radii_list.append(radii_[mask_at_box])
        ...
        radii = np.concatenate(radii_list).astype(np.float32)
        ...
        ray_o = ray_o[mask_at_box]
        ray_d = ray_d[mask_at_box]
        radii = radii[mask_at_box]
        coord = np.argwhere(mask_at_box.reshape(H, W) == 1)

    return rgb, ray_o, ray_d, near, far, coord, mask_at_box, radii
